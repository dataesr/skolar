...
ENTITIES_MAPPING = {"acknowledgement": ["funders", "infrastructures"]}
...
def compare(base_dir: Path, paragraph_type: str) -> tuple:
    llm_dir = base_dir / "llm" / paragraph_type
    llm2_dir = base_dir / "llm2" / paragraph_type
    ...
    for entity in ENTITIES_MAPPING.get(paragraph_type, []):
        agg_dict[entity] = (f"{entity}_count", "sum")

    def agg_by_pub(df: pd.DataFrame) -> pd.DataFrame:
        return df.groupby("pub_id").agg(**agg_dict).assign(processed=lambda d: d["total"] - d["empty"])
...
def compare_llm(args: dict):
    ...
    meta, per_pub = compare(Path(base_dir), paragraph_type)

    meta_path: Path = Path(output_dir) / "compare" / paragraph_type / "stats.json"
    pub_path: Path = Path(output_dir) / "compare" / paragraph_type / "publications.json"
